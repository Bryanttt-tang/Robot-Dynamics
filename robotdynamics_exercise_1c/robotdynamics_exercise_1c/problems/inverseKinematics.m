...
while (it==0 || (norm(dxe)>tol && it < max_it))
    ...
    % rotation error
    C_IE = jointToRotMat(q); %get current rotation matrix
    C_error=C_IE'*C_IE_des;
    E_dph=rotMatToRotVec(C_error); % rotation vector in current frame
    dph = C_IE*E_dph; 
    % pose error
    dxe =[dr;dph] ;
